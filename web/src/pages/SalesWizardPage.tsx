...
export function SalesWizardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5"><h3 className="text-xl font-bold">Riwayat penjualan</h3></div><div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 text-sm"><thead className="bg-slate-50"><tr><th className="px-5 py-3 text-left">Nomor</th><th className="px-5 py-3 text-left">Tanggal</th><th className="px-5 py-3 text-right">Total</th><th className="px-5 py-3 text-right">Outstanding</th><th className="px-5 py-3 text-left">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{salesQuery.data?.map((row) => <tr key={row.id} className="hover:bg-slate-50"><td className="px-5 py-3">{row.saleNumber}</td><td className="px-5 py-3">{new Date(row.saleDate).toLocaleString('id-ID')}</td><td className="px-5 py-3 text-right">{money.format(row.total)}</td><td className="px-5 py-3 text-right">{money.format(row.outstandingAmount)}</td><td className="px-5 py-3"><button className="rounded-lg border border-red-200 px-3 py-1 text-xs font-semibold text-red-600 disabled:opacity-40" disabled={row.status !== 1} onClick={() => confirm('Void penjualan ini?') && voidSale.mutate(row.id)}>Void</button></td></tr>)}</tbody></table></div></section>
    </div>
  )
}
